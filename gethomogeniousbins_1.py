def gethomogenousbins(samplenumber):
    ...
    for sample in samples[1:-1]: #Loop through each bin 
        ...
        if len(binned)!=0: 
            ...
            frequent = dict((k, v) for k, v in diction.items() if v >= 5) #Create new dictionary with all mate chromosome number that occur more than 2 times
            if 1 <= len(frequent) <= 5: #and not longer than 2
