import pysam
bam=pysam.Samfile("filter4-1.sorted.bam",'rb') #Open BAM-file that only has discordant reads of high quality
breaks=open("bin.txt", "r") #Open list of bin locations
bam_out = pysam.Samfile("filter4-2-old.bam", 'wb', template=bam) #Create output file 
new=0
filtered=0
lines=breaks.read()
...
for sample in samples[1:-1]: #Loop through each bin 
    ...
    if len(binned)!=0: 
        ...
        frequent = dict((k, v) for k, v in diction.items() if v >= 2) #Create new dictionary with all mate chromosome number that occur more than X times
        if len(frequent) ==1 or len(frequent) ==2: #and not longer than 2
            frequencies = frequent.keys() #Only take the chromosome number, not the count i.e. value
            for frequency in frequencies: 
                for bin in binned: #only take the reads that have a mate chromosome number that is frequent (more than X). 
